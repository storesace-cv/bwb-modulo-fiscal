...
- Compliance RM-REQ-001: Citação H — docs FE HML (`registar`/`solicitarSerie`/`listarSeries`/`listarFacturas`) + inventário `FE-RNG-*` extractado; AO-AGT-001 → `pending_validation`; AO-AGT-002 → `partial` (`requestID`); C-FE-001 (paths `/ws/` vs `/v1`); sem inventar códigos; sem AO-* confirmados.
