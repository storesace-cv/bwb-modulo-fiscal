...
- Compliance RM-REQ-001: Citação D expandida sobre `SAFTAO1.01_01.xsd` (`e9a938e1…`, elementos/linhas `AuditFile`/`InvoiceType`/`References`/`Hash`/`PaymentType`); AO-SAF-001/002 mantêm `pending_validation`; C-DOC-003 actualizado (`RC` em `PaymentType` ≠ fecho); sem AO-* confirmados.
