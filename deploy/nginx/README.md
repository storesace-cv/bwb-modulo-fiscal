...
- `nginx-deny-all <sha40>` — restore deny-all, cancel timer, retry probe until 403 (401 transient); fail-closed on miss
- `nginx-open-rollback-fire` — timer target: deny-all → `rolled_back` if 403; else terminal fail-closed state (never `armed`+timer inactive); noop if `confirmed`
