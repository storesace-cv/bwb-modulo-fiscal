...
| [ ] | RM-OPS-007 | Nginx open sandbox: proxy `/admin/v1/` + `/admin/ui` (fail-closed app; deny-all sem admin) | EM_CURSO | [deploy/nginx/open/bwb-fiscal-sandbox-tls.open.conf](deploy/nginx/open/bwb-fiscal-sandbox-tls.open.conf) · [docs/07-operations/rm-ops-006-sandbox-schema12-promotion-report.md](docs/07-operations/rm-ops-006-sandbox-schema12-promotion-report.md) | RM-OPS-006 + S3C2 confirmed | HTTPS `/admin/ui/login` HTML; ready público; documents 401; timer confirmado |
