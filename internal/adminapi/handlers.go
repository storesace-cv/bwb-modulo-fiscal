...
func Mount(mux *http.ServeMux, authn adminauth.Authenticator, h *Handler) {
	...
	mux.Handle("GET /admin/v1/establishments/{establishment_id}/document-availability", wrap(readCadastro(http.HandlerFunc(h.getDocumentAvailability))))
	mux.Handle("PUT /admin/v1/establishments/{establishment_id}/document-groups", wrap(writeCadastro(http.HandlerFunc(h.putDocumentGroupConfig))))
	mux.Handle("PUT /admin/v1/establishments/{establishment_id}/document-types", wrap(writeCadastro(http.HandlerFunc(h.putDocumentTypeConfig))))
	mux.Handle("POST /admin/v1/establishments/{establishment_id}/series", wrap(writeCadastro(http.HandlerFunc(h.createSeries))))
	mux.Handle("GET /admin/v1/establishments/{establishment_id}/series", wrap(readCadastro(http.HandlerFunc(h.listSeries))))
	mux.Handle("GET /admin/v1/series/{series_id}", wrap(readCadastro(http.HandlerFunc(h.getSeries))))
	mux.Handle("POST /admin/v1/series/{series_id}/transition", wrap(writeCadastro(http.HandlerFunc(h.transitionSeries))))
	...
	mux.Handle("GET /admin/v1/secret-refs", wrap(secadmWrite(http.HandlerFunc(h.listSecretRefMetadata))))
