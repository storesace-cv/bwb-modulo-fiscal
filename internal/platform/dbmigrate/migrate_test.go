...
func TestMigrationVersionParity(t *testing.T) {
	...
	if pgVers[len(pgVers)-1] != "0006" {
		t.Fatalf("latest version = %s, want 0006", pgVers[len(pgVers)-1])
	}
	if dbmigrate.ExpectedVersion != 6 {
...
func TestFiscalMigrateCLIOnlyUpVersion(t *testing.T) {
	...
	if !strings.Contains(string(out), "version=6") {
