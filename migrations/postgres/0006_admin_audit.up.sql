...
CREATE TABLE fiscal.admin_audit_events (
...
CREATE TRIGGER admin_audit_events_no_update
  BEFORE UPDATE ON fiscal.admin_audit_events
  FOR EACH ROW EXECUTE FUNCTION fiscal.reject_mutation();

CREATE TRIGGER admin_audit_events_no_delete
  BEFORE DELETE ON fiscal.admin_audit_events
  FOR EACH ROW EXECUTE FUNCTION fiscal.reject_mutation();

CREATE INDEX admin_audit_events_occurred_at_idx ON fiscal.admin_audit_events (occurred_at);
CREATE INDEX admin_audit_events_actor_subject_idx ON fiscal.admin_audit_events (actor_subject);
