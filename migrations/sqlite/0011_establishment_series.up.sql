...
CREATE TABLE establishment_series (
...
CREATE UNIQUE INDEX establishment_series_code_unique
  ON establishment_series (establishment_id, environment, code);

CREATE UNIQUE INDEX establishment_series_one_active_per_type_idx
  ON establishment_series (establishment_id, environment, codigo_canonico)
  WHERE status = 'active';

CREATE INDEX establishment_series_est_env_idx
  ON establishment_series (establishment_id, environment);
