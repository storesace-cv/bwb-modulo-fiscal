...
Hub owner-only (`RM-AGTPREP-015`…`018`): consolida selecção HML/PRD, catálogo de endpoints (URLs só alinhadas; C-FE-001 sem URL), scaffold JWS (`claims_status=pending_external`), inventário SecretStore mascarado, conectividade, e validação de bindings (ops+refs) — **fora** do nav operacional comum.
