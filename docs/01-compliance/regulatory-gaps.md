...
| GAP-004 | XSD oficial SAF-T (AO) | **Parcialmente mitigado:** XSD ASSOFT + LICENSE/NOTICE (`e9a938e1…`, `pending_validation`); Citação D com elementos/linhas (`InvoiceType` L2023+, `References` L1004+, `Hash` L1361+); `doc:Status=Development`; ZIP `local_only` | `AO-SAF-001`, `AO-SAF-002`, gerador/validador | Confirmação AGT + vetores dourados; fecho C-DOC-003 |
